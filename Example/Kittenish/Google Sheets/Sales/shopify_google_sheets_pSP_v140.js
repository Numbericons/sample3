...
const rowDetailKeys = ["total_price", "created_at", "cancelled_at", "closed_at", "confirmed", "id", "order_number"];
const rowSummaryKeys = ["total_price", "created_at", "cancelled_at", "closed_at", "confirmed"];
...
function orderDetail(data, object) {
  for (let i = 0; i < data.length; i++) {
    ...
    let hasAddr = data[i].customer && data[i].customer.default_address && data[i].customer.default_address.country === "United States";
    
    if (hasAddr) {
      row.country = data[i].customer.default_address.country;

      if (data[i].customer.default_address.country === "United States") {
        const zip = data[i].customer.default_address.zip;
        ...
        if (zip) {
          ...
          if (validZip) {
            row.zip = data[i].customer.default_address.zip;
          }
        }
      }
