  ...
  define(
    ...
    (record, search, runtime, error) => {
      ...
      function setSOValues(id, estimateId) {
        ...
        soValues = setChildValues(soValues, id)
        ...
        if (logOn) log.debug('Sales order status: ', status);

        if (status === 'Billed') salesOrder.setValue({
          fieldId: 'custbody_vel_ioct_process_complete',
          value: true,
        });
      ...
      function setChildValues(object, id) {
        var fulfillmentSearch = searchRelatedTrans('itemfulfillment', id);

        var fulfillArr = fulfillmentSearch.run().getRange({
          start: 0,
          end: 1
        });

        if (fulfillArr[0]) {
          object['custbody_vel_if_creation_date'] = adjustDate(fulfillArr[0].getValue({
              name: 'datecreated'
          }));
          log.debug('IF created date: ', object['custbody_vel_if_creation_date']);
  
          object['custbody_vel_if_date_shipped'] = adjustDate(fulfillArr[0].getValue({
            name: 'transhippeddate'
          }));
        }

        
        log.debug('IF Shipped date: ', object['custbody_vel_if_date_shipped']);

        var invoiceSearch = searchRelatedTrans('invoice', id);

        var invoiceArr = invoiceSearch.run().getRange({
          start: 0,
          end: 1
        });

        if (invoiceArr[0]) {
          object['custbody_vel_invoice_create_date'] = adjustDate(invoiceArr[0].getValue({
              name: 'datecreated'
          }));
      ...
      function adjustDate(date) { //addresses issue apparently related to timezone
        if (!date) return;
        const initial = new Date(date);

        return new Date(initial.setHours(initial.getHours() -2));
      }
      ...
      function getApprovedDate(type, id, oldVal, newVal) {
        ...
        try {
          ...
          return !isEmpty(date) ? adjustDate(date) : false;
        } catch (errorObj) {
          log.error({
      ...
      function searchRelatedTrans(type, id) {
        ...
        return search.create({
          ...
          [
              ...
              search.createColumn({name: "transhippeddate", label: "Picked Date"})
          ]
        });
      }
      ...
      function searchCreate() {
        return search.create({
          ...
          [
              ...
              ["mainline","is","T"],
              "AND",
              ["custbody_vel_ioct_process_complete","is","F"],
              "AND",
              ["datecreated","within","1/1/2022 12:00 am","1/31/2022 11:59 pm"]
          ],
          columns:
          [
              search.createColumn({name: "internalid", label: "Internal ID"}),
