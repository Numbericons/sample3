    ...
    function (record, search, url, format) {
        ...
        function setSalesOrderLineFlds(lineVals, lineFlds, soRec, svcItem, product, isZero, line, isCreate) {
            ...
            for (var i in lineFlds) {
                // var formatDate = typeof lineFlds[i] === 'string' && lineFlds[i].includes('date');
                // var val = formatDate ? format.format({value: lineVals[i], type: format.Type.DATE}) : lineVals[i];
                const dates = ['custcol_vel_report_sign_out_date_2', 'custcol_vel_menstrual_date', 'custcol_vel_est_delivery_date'];
                log.debug('Line field i type: ', typeof lineFlds[i]);
                log.debug('Line field i is in dates: ', dates.includes(lineFlds[i]));

                log.debug('Current line to set [Field: Value]: ', lineFlds[i] + ': ' + val);
