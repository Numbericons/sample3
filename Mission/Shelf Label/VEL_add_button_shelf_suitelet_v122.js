  ...
  (file, record, render, search, format) => {
    ...
    const getPrice = (item_record) => {
      ...
      for (let k=0; k<priceCount; k++){
        ...
        priceRet.retailPrice = setCurrency(priceRet.retailPrice);
        log.debug('Price ret with retail:', priceRet);

        var listPrice = priceRet.retailPrice = item_record.getSublistValue({
          sublistId: 'price',
          fieldId: 'price_1_',
          line: 0
        });

        if (priceRet.retailPrice !== listPrice) {
    ...
    const setCurrency = (amount) => {
      ...
      // if (array.length === 1) {
      //   return formatter.format({ number: amount }) // + '.00';
      // } else {
      //   let decimal = array[1];
      ...
      //   }
      //   return formatter.format({ number: parseFloat(array[0] + '.' + decimal) });
      // }
    }
