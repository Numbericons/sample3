  ...
  */
  ...
  define(
    ...
    "SuiteScripts/_work/srvc/design_to_build/code/nts_md_manage_item_master_v200"],
    (record, search, runtime, error, task, file, format, nts_md_manage_item_master) => {
      ...
      const getInputData = (context) => {
        try {
          ...
          // nukeRecords(); //removes all previous custom records to avoid overlap, can update function to target same criteria previous records
      ...
      const summarize = (summary) => {
        ...
        listCreate.setValue({
          fieldId: "custrecord_vel_price_list_last_run", 
          value: new Date()
        });
      ...
      function nukeRecords() {
        log.debug('nukeRecords entered!');
        ...
        log.debug('nukeRecords exit!');
        // var allCreateCustPL = createSearchPL();
        // log.debug('allCreateCustPL created!');

        // log.debug('Count of results : ', allCreateCustPL.runPaged().count);

        // allCreateCustPL.run().forEach(function (result) {
      ...
      function createSearchPL() {
        return search.create({
          type: "customrecord_vel_customer_pr_rule",
          filters:
          [
              ["lastmodified","onorafter","8/22/2022 2:15 pm"]
          ],
          columns:
          [
      ...
      function query_price_rules_dom(customer, trandate) {
        var search_obj = search.create({
          ...
          columns: [
            ...
            search.createColumn({
              name: "scriptid", label: "Script ID",
            },),
            search.createColumn({
              name: "custrecord_nts_pr_start_date", label: "Start Date",
            },),
            search.createColumn({
              name: "custrecord_nts_pr_end_date", label: "End Date",
            },),
            search.createColumn({
              name: "custrecord_nts_pr_customer", label: "Customer",
            },),
            search.createColumn({
              name: "custrecord_nts_pr_item",
              sort: search.Sort.ASC,
            ...
            search.createColumn({
              name: "custrecord_nts_pr_calculation_method", label: "Calculation Method",
            },),
            search.createColumn({
              name: "custrecord_nts_pr_calculation_basis", label: "Calculation Basis",
            },),
            search.createColumn({
              name: "custrecord_nts_pr_adjust_pct", label: "Adjust %",
            },),
            search.createColumn({
              name: "custrecord_nts_pr_adjust_amt", label: "Adjust $",
            },),
            search.createColumn({
              name: "custrecord_nts_pr_tier_basis", label: "Tier Basis",
            },),
          ],
        },);
