...
define(['N/error','N/search','N/record','N/workflow','N/runtime'],function(error,search,record,workflow,runtime) {
  function beforeSubmit(context) {
    ...
    // var contextInternalId = contextRecord.getValue({fieldId: 'internalid'});
    ...
    var itemsAddedToOrderIds = [
      {item: 7144, quantity: 2}, 
      {item: 7145, quantity: 1}
    ];
    ...
    //log.debug('Script parameters','orderStatus='+orderStatus+' eTailChannel='+channel+' NthOrderCount='+custOrderCount);
    //
    //if (channel=='Shopify' && custOrderCount==specifiedOrderCount){
    //above is when custOrderCount matters
    //
    if (channel=='101'){ //'101' is the id of Shopify
      ...
      if(promoOneAddItemsToOrder==1 || promoTwoAddItemToOrder==1 || promoThreeAddItemsToOrder==1){
        ...
        log.debug('item lines : ', lines);

        if(promoOneAddItemsToOrder==1){
          log.audit('Adding Item to Order','item='+itemDebugName+', orderStatus='+orderStatus+' eTailChannel='+channel+' NthOrderCount='+custOrderCount);
        ...
        if(promoTwoAddItemToOrder==1){
          log.audit('Adding Item to Order','item='+promoTwoItemDebugName+', orderStatus='+orderStatus+' eTailChannel='+channel+' NthOrderCount='+custOrderCount);
          //add the specified item to the order for when custOrderCount=specifiedOrderCount
          currentRecord.setSublistValue({
            sublistId: 'item',
            fieldId: 'item',
        ...
        if(promoThreeAddItemsToOrder==1){
          log.audit('Adding Item to Order','item='+promo3itemDebugName+', orderStatus='+orderStatus+' eTailChannel='+channel+' NthOrderCount='+custOrderCount);
          //add the specified item to the order for when custOrderCount=specifiedOrderCount
          ...
          for (var i=0; i<itemsAddedToOrderIds.length; i++){
            currentRecord.setSublistValue({
              ...
              fieldId: 'item',
              line: lines - 1 + i,
              value: itemsAddedToOrderIds[i].item
            });
            currentRecord.setSublistValue({
              sublistId: 'item',
              fieldId: 'quantity',
              line: lines - 1 + i,
              value: itemsAddedToOrderIds[i].quantity
            });
            currentRecord.setSublistValue({
              sublistId: 'item',
              fieldId: 'rate',
      ...
      log.debug('No Items to Add to Order','internalId='+contextInternalId+' orderStatus='+orderStatus+' eTailChannel='+channel+' NthOrderCount='+custOrderCount);
    }
    return true;
