...
define(['N/error','N/search','N/record','N/workflow','N/runtime'],function(error,search,record,workflow,runtime) {
  function afterSubmit(context) {
    ...
    var itemsAddedToOrderIds = [
      {item: 7194, quantity: 2}, 
      {item: 7195, quantity: 1}
    ];
    ...
    var storeId = currentRecord.getValue({
      fieldId: 'custbody_celigo_shopify_store_id'
    });

    const lsfShopifyStoreId = "13083049"
    if (storeId !== lsfShopifyStoreId) {
      log.debug('Store ID not LSF!');
      return; //promotions via this script are only applicable to LSFas of 9/2022.
    }
    ...
    if (channelText=='Shopify'){
      ...
      var promoThreeAddItemsToOrder = 0;
      var promoThreeExcludedProductsFound = 0;
      var promoThreeEndDateTime = '9/15/2022 23:59:00';
