  ...
  function onAction(scriptContext) {
    ...
    if (scheduledToRun){
      try {
        ...
        /*var salesOrderRecord= record.load({//This was the variable used as the record id when ran at afterSubmit
          type: record.Type.SALES_ORDER,
          id: internalOrderId,
          isDynamic: false
        });*/

        var docNum = newRecord.getValue('tranid');

        var savedOosText = newRecord.getValue(oosListCustomFieldId);
        ...
        if(oosText==savedOosText){
          ...
          const field = newRecord.setValue({
        ...
        /*if (needToSave){//When run on afterSubmit it needed to be saved
          log.audit({
