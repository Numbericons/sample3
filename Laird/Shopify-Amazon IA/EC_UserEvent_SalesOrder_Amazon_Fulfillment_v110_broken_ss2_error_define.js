...
define(
  ...
  function (record, log, runtime, search, format, task, error) {
    ...
    function onAfterSubmit(type) {
      try {
        ...
        if (backOrderedItemExists && salesOrderRecord.custbody_lsf_amazon_channel == "2") {
          log.error('salesOrderRecord', salesOrderRecord);
          var unfulfilledFBAOrder = error.create({
            name: 'Unfufilled_FBA_Order',
            msg: 'Unfulfilled FBA Order: ' + JSON.stringify(salesOrderRecord)
          })
          throw unfulfilledFBAOrder;
        }
