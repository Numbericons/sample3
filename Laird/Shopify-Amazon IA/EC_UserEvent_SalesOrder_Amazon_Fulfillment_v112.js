...
(function (EC_SO_AMZ_FUL) {
  function onAfterSubmit(type) {
    try {
      ...
      var salesOrderRecord = nsdal.loadObject(nlapiGetRecordType(), nlapiGetRecordId(), ['tranid', 'custbody_lsf_amazon_channel', 'status', 'trandate']).withSublist('item', ['item', 'quantitybackordered']);
      ...
      var backOrderedItemExists = _.find(salesOrderRecord.item, function (item) { return _.toNumber(item.quantitybackordered || 0) > 0; });
      log.debug('backOrderedItemExists', 'value='+backOrderedItemExists);

      if (backOrderedItemExists && salesOrderRecord.custbody_lsf_amazon_channel == "2") {
        nlapiLogExecution('ERROR', 'salesOrderRecord', JSON.stringify(salesOrderRecord));
        throw nlapiCreateError('Unfufilled_FBA_Order', 'Unfulfilled FBA Order: ' + JSON.stringify(salesOrderRecord), false);
      }

      // only fulfill the sales order if these criteria work out
      if (salesOrderRecord.status == 'Pending Fulfillment' && !backOrderedItemExists && salesOrderRecord.custbody_lsf_amazon_channel == '2')
        fulfillSalesOrder(salesOrderRecord);
    }
    catch (e) {
      log.error('unexpected error- EC_SO_AMZ_FUL.onAfterSubmit', EC.getExceptionDetail(e));
    }
  }
  ...
  function fulfillSalesOrder(salesOrderRecord) {
    ...
    var itemFulfillmentRecord = nsdal.fromRecord(nlapiTransformRecord('salesorder', salesOrderRecord.getId(), 'itemfulfillment', { recordmode: 'dynamic' }), ['createdfrom', 'shipstatus', 'trandate']);
    itemFulfillmentRecord.withSublist('item', ['item', 'quantity', 'itemreceive', 'inventorydetailreq', 'itemtype']);
    ...
    var lotNumberResults = lazy.loadSearch('', 'customsearch_ec_lot_no_avail_amz')
    ...
    _.forEach(itemFulfillmentRecord.item, function (lineItem, index) {
      ...
      if (lineItem.inventorydetailreq == 'F' || lineItem.item == "2524"  || lineItem.item == "2475" || lineItem.itemtype == "Kit") {
        ...
        var subRecord_1 = itemFulfillmentRecord.createCurrentLineItemSubrecord('item', 'inventorydetail');
        ...
        _.forEach(lotNumbers, function (lotNumber, iaIndex) {
          ...
          subRecord_1.setCurrentLineItemValue('inventoryassignment', 'issueinventorynumber', lotNumber.id);
          subRecord_1.setCurrentLineItemValue('inventoryassignment', 'quantity', lotNumber.quantity);
  ...
  function getLotNumbers(lotNumberResults, lineItem) {
    ...
    var lotNumberMatches = _.filter(lotNumberResults, function (lotNumberResult) { return lotNumberResult.item == lineItem.item; });
    ...
    _.forEach(lotNumberMatches, function (lotNumberMatch) {
      ...
      remainderQuantity = Math.max(0, _.toNumber(remainderQuantity) - _.toNumber(lotNumberMatch.quantityavailable));
