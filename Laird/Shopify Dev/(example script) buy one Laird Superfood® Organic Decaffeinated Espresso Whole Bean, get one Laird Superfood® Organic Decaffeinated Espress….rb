...
eligible_items = Input.cart.line_items.select do |line_item|
  ...
  !product.gift_card? && product.id == 6382402241
end

discounted_line_items = partition(Input.cart, eligible_items)
discounted_line_items.each do |line_item|
  line_item.change_line_price(Money.zero, message: "Buy one Laird Superfood® Organic Decaffeinated Espresso Whole Bean, get one Laird Superfood® Organic Decaffeinated Espresso Whole Bean free")
end
