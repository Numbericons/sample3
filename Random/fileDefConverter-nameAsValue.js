...
function convertDefs(origFileDef) {
  //apparently for imports rules[0].maxOccurence has to be 2 instead of 1 to work
  origFileDef.fileDefinition.rules[0].maxOccurrence = 2
    for (let i in origFileDef.fileDefinition.rules) {
        if (origFileDef.fileDefinition.rules[i].elements) {
            for (let j = 1; j < origFileDef.fileDefinition.rules[i].elements.length; j++) {
              let tempName = JSON.parse(JSON.stringify(origFileDef.fileDefinition.rules[i].elements[j].value))
              let tempValue = JSON.parse(JSON.stringify(origFileDef.fileDefinition.rules[i].elements[j].name))
                origFileDef.fileDefinition.rules[i].elements[j].name = tempName
                origFileDef.fileDefinition.rules[i].elements[j].value = '{{{[' + tempValue + ']}}}'
            }
        }
...
function cleanChildren(arr) {
    //if container is marked, we want a relativeDataPath with first element value
    if (arr.container) {
        arr.relativeDataPath = arr.children[0].elements[0].value
    }
    for (let k in arr.elements) {
        ...
        let tempName = JSON.parse(JSON.stringify(arr.elements[k].value))
        let tempValue = JSON.parse(JSON.stringify(arr.elements[k].name))
        arr.elements[k].name = JSON.parse(JSON.stringify(tempName))
        arr.elements[k].value = '{{{[' + tempValue + ']}}}'
    }
        if (arr.children) {
