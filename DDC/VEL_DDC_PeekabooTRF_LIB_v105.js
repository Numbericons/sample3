    ...
    function (record, search, url, format) {
        ...
        function setSalesOrderFlds(headerVals, headerFlds, lineVals, lineFlds, soRec, isCreate, item, isZero, line, amazonUpdate) {
            ...
            for (var i in headerFlds) {
              log.debug('Current Header to set [Field: Value]: ', headerFlds[i] + ': ' + headerVals[i]);
              const dateHeader = headerFlds[i] === 'custbody_vel_due_date';
              var header = dateHeader ? format.format({value: headerVals[i], type: format.Type.DATE}) : headerVals[i];
        ...
        function setSalesOrderLineFlds(lineVals, lineFlds, soRec, svcItem, product, isZero, line, isCreate) {
            ...
            for (var i in lineFlds) {
              var dateField = dateFields(lineFlds[i]);
              var val = dateField == 'date' ? new Date(lineVals[i]) : lineVals[i];

              log.debug('Line field i type: ', typeof lineFlds[i]);
              log.debug('Line value i type: ', typeof lineVals[i]);
              log.debug('Line val: ', val);
              log.debug('Line field i is a date field? : ', dateField);
              log.debug('Current line to set [i Field: Value]: ', i + ' ' + lineFlds[i] + ': ' + val);
        ...
        function dateFields(string) {
          var dates = ['custcol_vel_report_sign_out_date_2', 'custcol_vel_menstrual_date', 'custcol_vel_est_delivery_date'];
          
          for (var k=0; k<dates.length; k++){
            if (string === dates[k]) return 'date';
          };
